...
if (isset($_POST['sub_code']) && isset($_POST['sub_name']))
			{
				...
				$sub_name = $_POST['sub_name'];
				 ...
				 $query = "INSERT INTO `feedback`.`paper` (`papercode`, `papername`) VALUES ('$sub_code', '$sub_name');";
...
?>
   <h3>Availble Subjects</h3> 
  <?php
  	$query = "SELECT * FROM `feedback`.`paper`";
        $result = mysqli_query($conn,$query);
  ...
  No Subjects Added Yet!
		...
		echo" <table class='table table-striped' id='result2'>
    <thead>
      <tr>
        <th>Sl No.</th>
        <th>Subject Code</th>
    ...
    <tbody>
	  ...
	  $ct=1;
        while($row = $result->fetch_array()) {
			print '<td>'.$ct++.'</td>';
			print '<td>'.$row["papercode"].'</td>';
			print '<td>'.$row["papername"].'</td>';
			echo "<td><a href='deletesubject.php?id=" . $row['papername'] . "'>Delete</td>";
   print'</tr>';
		}
   echo " </tbody>
  </table>";
